...
public final class FileUtil {
    ...
    public static void copyDirectoryContents(final Path input, final Path output, final String... exclusions) throws IOException {
        createDirectory(output);
        final List<Path> tree = directoryTree(input, exclusions);

        for (Path path : tree) {
            PathUtils.copyFileToDirectory(path, output);
        }
    }
    ...
    /**
     * Creates a list of paths in the provided directory.
     *
     * @param directory The directory.
     * @param exclusions A list of regexes indicating excluded paths.
     * @return The list.
     * @throws IOException exception.
     */
    public static List<Path> directoryTree(final Path directory, final String... exclusions) throws IOException {
        try (Stream<Path> stream = Files.walk(directory)) {
            if (exclusions != null) {
                return stream.filter(path -> notExcluded(getAbsolutePath(path), exclusions)).toList();
            } else {
